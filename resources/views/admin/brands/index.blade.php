                        ...
                        <tbody>
                            @foreach ($vehicleBrands as $index => $brand)
                                <tr>
                                    ...
                                    <td>{{ $brand->vehicleType->name ?? '-' }}</td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('vehicle-brands.edit', $brand->id) }}" class="btn btn-sm btn-warning">
                                                <i class="bi bi-pencil-square"></i> Edit
                                            </a>

                                            {{-- Form hapus dengan SweetAlert2 --}}
                                            <form action="{{ route('vehicle-brands.destroy', $brand->id) }}" method="POST" class="delete-form">
                                                @csrf
                                                @method('DELETE')
...
<script>
    ...
    $(document).ready(function () {
        $('.btn-delete').click(function(e) {
            ...
            Swal.fire({
                title: 'Yakin ingin menghapus merek ini?',
                icon: 'warning',
